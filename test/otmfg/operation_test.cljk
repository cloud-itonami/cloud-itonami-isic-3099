(ns otmfg.operation-test
  "Smoke tests for the compiled OtherTransportOperationActor graph
  itself (build + one happy path per op). The governor's full rule
  contract (HARD holds, escalation, phase gating) is exercised in
  `otmfg.governor-contract-test`; the Store contract in
  `otmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "OtherTransportOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "assembly-jig-001" :severity :moderate :description "axle mount anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
