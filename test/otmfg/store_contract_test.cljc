(ns otmfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `otmfg.store` ns docstring for why a second (Datomic-backed)
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (true? (:verified? (store/equipment-unit s "assembly-jig-001"))))
    (is (true? (:registered? (store/equipment-unit s "assembly-jig-001"))))
    (is (false? (:verified? (store/equipment-unit s "testbench-002"))))
    (is (false? (:registered? (store/equipment-unit s "testbench-002"))))
    (is (= ["assembly-jig-001" "testbench-002"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest fresh-store-has-no-batches-or-equipment
  (let [s (store/mem-store)]
    ...
    (is (nil? (store/equipment-unit s "assembly-jig-001")))))

(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-category :animal-drawn-wagon}})
    (is (= :animal-drawn-wagon (:product-category (store/batch s "batch-001"))))
...
(deftest maintenance-schedule-commits-and-advances-sequence
  (testing "commit-record! (like every sibling actor's own MemStore) returns the store `s`, not the domain result -- inspect the store directly, matching the discipline the actor's own :commit node relies on"
    (let [s (seeded)]
      ...
      (is (= "assembly-jig-001" (:equipment-id (store/maintenance s "mnt-1"))))
