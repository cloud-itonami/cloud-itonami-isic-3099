(ns otmfg.store
  "SSoT for the other-transport-equipment (animal-drawn and
  hand-propelled vehicle) plant-operations coordination actor, behind
  ...
  self-contained (no external other-transport-equipment-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             ...
                             assembly-defect-rate record. `:verified?`
    ...
    - `equipment`         -- an assembly-line unit's own record.
    ...
    - `maintenance`       -- a scheduled assembly-line maintenance-
                             window DRAFT against a piece of equipment
                             (`otmfg.registry`'s `register-maintenance`).
                             Dedicated `:scheduled?` double-schedule
                             guard (never a `:status` value -- the same
    ...
    - `shipments`         -- a proposed outbound other-transport-
                             equipment shipment DRAFT (`otmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (otmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (otmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-category :animal-drawn-cart
                :frame-size "standard" :weight-capacity-kg 400.0
                :quantity-units 500.0 :assembly-defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-category :hand-truck
                :frame-size "compact" :weight-capacity-kg 150.0
                :quantity-units 80.0 :assembly-defect-rate-percent 0.3
                :verified? true :registered? true
                :shipped-units 75.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-category :animal-drawn-wagon
                :frame-size "heavy-duty" :weight-capacity-kg 1800.0
                :quantity-units 600.0 :assembly-defect-rate-percent 2.3
                :verified? false :registered? false
                :shipped-units 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"assembly-jig-001" {:id "assembly-jig-001" :kind :chassis-assembly-jig
                        :verified? true :registered? true
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `otmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `otmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  set -- one verified+registered animal-drawn-cart batch with shipping
  headroom (schedulable), one verified+registered hand-truck batch
  that is nearly fully shipped (a small new shipment blows through its
  own logged production quantity -- HARD hold), one
  UNVERIFIED/unregistered batch (blocks any shipment coordinated
  against it); one verified+registered chassis-assembly-jig unit
  (schedulable for maintenance), one UNVERIFIED/unregistered
  structural-test-bench unit (blocks any maintenance scheduling
