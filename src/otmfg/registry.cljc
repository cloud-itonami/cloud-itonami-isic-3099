(ns otmfg.registry
  "Pure-function domain logic for the other-transport-equipment
  (animal-drawn and hand-propelled vehicle) plant-operations
  coordination actor -- equipment/batch verification, shipment-
  quantity recompute, product-category validation, weight-capacity
  plausibility validation, assembly-defect-rate plausibility
  ...
  pre-existing `kotoba-lang/otmfg`-style capability library to wrap
  (verified: no such repo exists). The domain logic therefore lives
  here as pure functions, re-verified INDEPENDENTLY by
  `otmfg.governor` -- the same 'ground truth, not self-report'
  ...
  coordinated shipment), not the act of actuating assembly-line
  equipment or dispatching a real freight carrier, and never the act
  of issuing a transport-equipment safety/roadworthiness certification
  (this actor NEVER does any of those -- see README `What this actor
  does NOT do`).

  SCOPE: ISIC 3099 (Manufacture of other transport equipment n.e.c.)
  is a RESIDUAL class -- animal-drawn vehicles (carts, wagons,
  carriages) and hand-propelled vehicles (hand-carts, hand-trucks,
  wheelbarrows, rickshaws, sledges, toboggans, pushcarts) assembled on
  a chassis/axle/wheel/body assembly line and inspected on structural/
  load test benches. Bicycles and invalid carriages (manual/power
  wheelchairs, mobility scooters) are EXCLUDED -- those are ISIC 3092
  (`cloud-itonami-isic-3092`), a distinct, already-implemented actor.
  This actor coordinates the back-office record-keeping around the
  other-transport-equipment plant (production-batch logging,
  maintenance scheduling, safety-concern flagging, shipment
  coordination) -- it never touches the assembly-line equipment
  directly, and it never stands in for the certification/regulatory
  body that issues transport-equipment safety/roadworthiness
  approval marks.")
...
(def valid-product-categories
  ...
  may declare -- spans both animal-drawn vehicles and hand-propelled
  vehicles, the two product families this residual ISIC 3099 vertical
  covers (explicitly EXCLUDING bicycles and invalid carriages, which
  are ISIC 3092). Anything else is a fabricated/unrecognized product
  category -- the governor HARD-holds rather than let an invented
  category pass through."
  #{:animal-drawn-cart :animal-drawn-wagon :animal-drawn-carriage
    :hand-cart :hand-truck :wheelbarrow :rickshaw :sledge :toboggan
    :pushcart})

(def weight-capacity-min-kg
  "Physical floor for an other-transport-equipment unit's own rated
  maximum load capacity."
  0)

(def weight-capacity-max-kg
  "Physical ceiling for an other-transport-equipment unit's own rated
  maximum load capacity (spans light hand-carts/wheelbarrows through
  heavy-duty animal-drawn farm wagons). A reading above this is an
  implausible/fabricated rating, not a real standardized load-capacity
  spec."
  3000)

(def defect-rate-min-percent
  "Physical floor for a batch's own assembly defect-rate reading (zero
  defects is the best possible outcome, never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own assembly defect-rate reading -- a
...
(defn batch-verified?
  ...
  (i.e. its product-category/weight-capacity/quantity/assembly-defect-
...
(defn product-category-valid?
  ...
  values (animal-drawn vehicles or hand-propelled vehicles)? nil/blank
  is treated as invalid (a production-batch patch must declare a real
  product category, not omit it silently)."
  [product-category]
  (contains? valid-product-categories product-category))

(defn weight-capacity-valid?
  "Is `weight-capacity-kg` a physically plausible rated maximum load
  capacity? Rejects nil, non-numbers, negative values, and values
  beyond `weight-capacity-max-kg` -- a fabricated or sensor-error
...
(defn defect-rate-valid?
  "Is `percent` a physically plausible batch assembly defect-rate
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER a transport-equipment safety/roadworthiness
...
(defn register-maintenance
  ...
  assembly-line maintenance window against a verified, registered
  piece of equipment. Pure function -- does not actuate the
  assembly-line equipment or execute any maintenance; it builds the
  RECORD a plant coordinator would keep. `otmfg.governor`
  ...
  the assembly-line equipment (see README `Actuation`), before this is
...
(defn register-shipment
  ...
  outbound other-transport-equipment shipment against a verified,
  ...
  keep. `otmfg.governor` independently re-verifies the shipment's own
