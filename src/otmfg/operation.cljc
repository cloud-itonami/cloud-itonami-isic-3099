(ns otmfg.operation
  "OtherTransportOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (OtherTransportAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Other Transport Equipment Plant Operations Governor (:govern) and
...
(defn build
  "Compiles an OtherTransportOperationActor graph bound to `store`.
  opts:
    :advisor      -- an `otmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; OtherTransportAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Other Transport Equipment Plant Operations Governor -- independent censor.
      (g/add-node :govern
