(ns otmfg.advisor
  "OtherTransportAdvisor -- the *contained intelligence node* for the
  other-transport-equipment (animal-drawn and hand-propelled vehicle)
  plant-operations coordination actor.

  It normalizes production-batch patches (product-category/weight-
  capacity/quantity/assembly-defect-rate), drafts an assembly-line
  maintenance scheduling proposal against a piece of equipment, drafts
  a safety-concern flag, and drafts an outbound other-transport-
  equipment shipment coordination proposal against a production batch.
  CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real assembly-line-equipment actuation,
  freight dispatch, or transport-equipment safety/roadworthiness
  certification -- see README `What this actor does NOT do`. Every
  output is censored downstream by `otmfg.governor` before anything
  touches the SSoT.
  ...
  contract every caller of this actor agrees to) -- `otmfg.governor`
...
(defn- log-production-batch
  ...
  category, weight-capacity, quantity, assembly-defect-rate or
...
(defn- schedule-maintenance
  "Draft an assembly-line maintenance-window scheduling proposal
  against a piece of equipment. The advisor reports what it can see
  (equipment verified?/registered?) in its rationale, but
  `otmfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a materials-safety/structural-integrity concern. ALWAYS
  ...
  `otmfg.phase`: no phase ever adds this op to a phase's `:auto` set;
  `otmfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound other-transport-equipment shipment coordination
  ...
  `otmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは動物牽引車・手押し車製造工場プラント運用コーディネーターの"
       "助言者です。"
       ...
       "組立ライン・治具等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "輸送機器安全・走行適性証明を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `otmfg.governor`
